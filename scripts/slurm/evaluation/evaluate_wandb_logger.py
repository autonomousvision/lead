...
class WandBLogger:
    def __init__(
        self,
        closed_loop_config: EvaluationConfig,
        num_routes,
    ...
    def log_job(
        ...
        if is_on_slurm():
            ...
            if self.config_slurm.evaluation_dataset in ["bench2drive", "longest6"]:
                self.run.log(
            ...
            elif self.config_slurm.evaluation_dataset in ["fail2drive"]:
                self.run.log(
                    {"ids/route_id": int(carla_route_id.rsplit("_", 1)[-1])},
                    commit=False,
                )
            else:
                raise ValueError(
