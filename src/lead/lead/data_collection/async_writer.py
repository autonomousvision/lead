...
class AsyncWriter:
    ...
    Items are processed in submission order. The queue is kept tiny because
    each item can hold large sensor arrays: if the writer cannot keep up,
    :meth:`submit` blocks and the caller degrades to synchronous behavior.
    A write error is re-raised on the next :meth:`submit` so the run fails
    loudly instead of producing an incomplete log.
    """
