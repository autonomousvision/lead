"""Longitudinal planning for the expert: target-speed selection.

Forecasts ego and surrounding actors with kinematic models, checks future
bounding-box overlaps, and computes IDM target speeds with respect to leading
vehicles, vulnerable road users, red lights and stop signs.
"""
...
from lead.common import constants, weather
from lead.common.weather import WeatherVisibility
...
class SpeedPlanningMixin:
    """Target-speed computation, mixed into :class:`lead.lead.expert.Expert`."""
    ...
    def _plan_target_speed_limit(self) -> float:
        ...
        self.weather_setting = expert_weather.get_weather_name(
            self.carla_world.get_weather(),
            self.config_expert,
        )
        self.visual_visibility = int(
            weather.WEATHER_VISIBILITY_MAPPING[self.weather_setting],
        )
