...
def _lateral_pid_step(
    ...
    """One ExpertLateralPIDController.step with the window kept in an array.

    Mirrors the reference implementation exactly (same operation order) so the
    forecast matches the pure-Python controller bit-for-bit.
    """
    speed_kph = speed * 3.6
...
def _throttle_extrapolation(
    ...
    """ExpertLongitudinalController.get_throttle_extrapolation, scalarized."""
...
def forecast_ego_agent_kernel(
    ...
    """Forecast the ego agent without hazards over the future frames.

    Replicates the pure-Python loop of PID steering, throttle extrapolation,
    kinematic bicycle step and privileged route-index advance. The controller
    window and route index are evolved locally, leaving the real ones untouched.
