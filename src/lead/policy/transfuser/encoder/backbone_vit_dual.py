"""Throughput variant: patch-32 ViTs on both the image and the LiDAR branch.

Both branches are single-pass ViTs tapped at their last four blocks, all taps
at the patch-grid resolution and the embedding width, so the per-stage anchor
pooling is nearly free and the fusion transformers all run at one width. The
taps come from single passes, so cross-modal residuals do not feed later
stages on either branch; each stage fuses its own tap pair and the last fused
pair is the output.

Both architectures stay config-selected: point ``image_architecture`` and
``lidar_architecture`` at patch-32 ViTs (e.g. ``vit_small_patch32_384``) when
selecting this backbone. The LiDAR ViT is built from scratch on the raster's
channel count, like the base LiDAR CNN.
"""
...
class DualVitPatch32Backbone(TransfuserBackbone):
    """TransfuserBackbone with single-pass ViT branches for both modalities."""

    def __init__(self, lead_config: LeadConfig) -> None:
        """Duplicate of the base constructor with two ViT branches.
        ...
        lidar_infos = self.lidar_encoder.feature_info.info[-4:]
        self.num_lidar_features = lidar_infos[3]["num_chs"]
        self.lidar_channel_to_img = nn.ModuleList(
            [
                nn.Conv2d(
                    lidar_infos[i]["num_chs"],
    ...
    def _forward(
        ...
        """
        Image + LiDAR feature fusion with single-pass ViT taps on both branches.
        """
        ...
        lidar_taps = self.lidar_encoder(lidar)

        image_features = image_taps[0]
        lidar_features = lidar_taps[0]
        for i in range(4):
