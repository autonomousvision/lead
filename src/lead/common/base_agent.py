...
from lead.common.control.kalman_filter import KalmanFilter
from lead.common.planning import RoutePlanner
from lead.common.sensors import ransac_fast
from lead.config import LeadConfig
from lead.lead.utils.expert_utils import step_cached_property
...
class BaseAgent:
    ...
    def setup(self, lead_config: LeadConfig, sensor_agent: bool = False) -> None:
        ...
        self.lidar_pc_queue = deque(
            maxlen=self.config_expert.data_collection.lidar_stack_size,
        )  # For stacking LiDAR
        self.radar_pc_queue = deque(
            maxlen=2 * self.config_expert.data_collection.lidar_stack_size,
    ...
    def tick(self, input_data: dict, use_kalman_filter: bool = True) -> dict:
        ...
        if self.config_expert.sensor_rig.use_radars:
            ...
            if self.config_expert.sensor_rig.save_radar_pc_as_lidar:
                input_data["lidar"] = np.concatenate(
                    [input_data["lidar"], self.radar_pc_queue[-1]],
                    axis=0,
                )
                if self.config_expert.sensor_rig.duplicate_radar_near_ego:
                    radar_near_ego = self.radar_pc_queue[-1][
                        np.linalg.norm(self.radar_pc_queue[-1][:, :2], axis=1)
                        < self.config_expert.sensor_rig.duplicate_radar_radius
                    ]
                    radar_near_ego = np.concatenate(
                        [radar_near_ego]
                        * self.config_expert.sensor_rig.duplicate_radar_factor,
                        axis=0,
                    )
                    input_data["lidar"] = np.concatenate(
        ...
        lidar_x, lidar_y = input_data["lidar"][:, 0], input_data["lidar"][:, 1]
        if self.config_expert.storage.save_only_non_ground_lidar:
            ground_mask = ransac_fast.remove_ground_fast(
                input_data["lidar"],
                self.config_expert,
            )
            ...
            if (
                ...
                and self.config_expert.sensor_rig.save_radar_pc_as_lidar
            ):
        ...
        if self.config_expert.sensor_rig.save_lidar_only_inside_bev:
            ...
            inside_mask = (
                (self.config_expert.data_collection.min_x_meter <= lidar_x)
                & (lidar_x <= self.config_expert.data_collection.max_x_meter)
                & (self.config_expert.data_collection.min_y_meter <= _lidar_y)
                & (_lidar_y <= self.config_expert.data_collection.max_y_meter)
                & (self.config_expert.storage.min_height_lidar <= lidar_z)
                & (lidar_z <= self.config_expert.storage.max_height_lidar)
            )
        ...
        if self.sensor_agent:
            # The model consumes all cameras stitched side by side
    ...
    def accumulate_lidar(self) -> jt.Float[npt.NDArray, "n 4"]:
        ...
        for i, lidar_pc in enumerate(lidar_queue):
            if i > 0 and not self.config_expert.sensor_rig.lidar_accumulation:
                break
