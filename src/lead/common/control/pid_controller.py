...
class LateralPIDController:
    ...
    def __init__(self, config: ControllerConfig) -> None:
        ...
        Args:
            config: Controller configuration containing PID parameters.
        """
...
class ExpertLateralPIDController:
    ...
    def __init__(self, config: ExpertConfig) -> None:
        ...
        self.lateral_pid_default_lookahead = (
            self.config.pid.lateral_pid_default_lookahead
        )
        self.lateral_pid_speed_threshold = self.config.pid.lateral_pid_speed_threshold
    ...
    def step(
        ...
        if inference_mode:
            ...
            lookahead_distance = (
                ...
                / self.config.simulation.points_per_meter
            )  # range [2.4, 10.5]
...
def get_throttle(
    ...
    speed = speed * 3.6
    target_speed = target_speed * 3.6
    params = config.pid.longitudinal_params
    speed_error = target_speed - speed

    # maximum acceleration 1.9 m/tick
    if speed_error > config.driving.longitudinal_max_accelerations:
        return 1.0, False
