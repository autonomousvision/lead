"""Geometry of the perturbated sensor rig's virtual ego frame ("view frame").

Re-expresses ego-frame quantities (labels, lidar, routes, waypoints) in the
view frame when training on perturbated views; for the normal view every
...
def derive_rig_perturbation(
    ...
    The perturbated split stores the perturbated ``camera_to_imu_se3`` (see
    ``CameraRecorder``); the perturbation itself is not logged. The relative
    transform between the two extrinsics of the same camera is the rig
    perturbation conjugated into the ISO IMU frame — undo the rear-axle shift
    and the CARLA→ISO mirroring to get back the sampled scalars.
